...
def parse_args():
    ...
    parser.add_argument('--output_dir', default="/home/airlab/storage/airlab/OWID/P1_raw", help="Path to where the final files, will be saved")
    parser.add_argument('--synset_id', default="02691156", help="folder id to be rendered")
...
def main():
    args = parse_args()
    bproc.init()
    # /media/airlab/18b518f5-c2be-4c8c-be4d-aaae1d4c276f/airlab/ShapeNetCore.v2/04379243/models/model_normalized.obj
    # debugpy.listen(5670)
    # debugpy.wait_for_client()
    all_source_id = os.listdir(os.path.join(args.shapenet_path, args.synset_id))[0:100]
    skipped_id = ['02880940', '04263257', '02946921', '02823510', '04255586', '03797390', '02824058', '03063599']
    ...
    # load the ShapeNet object into the scene
        if os.path.isdir(os.path.join(args.output_dir, args.synset_id, source_id+'_00', 'test_video')):
            print('Skipping {}_{}'.format(args.synset_id, source_id))
            continue
        shapenet_obj = [bproc.loader.load_shapenet(args.shapenet_path, used_synset_id=args.synset_id, used_source_id=source_id)]

        # Randomize materials and set physics
        for obj in (shapenet_obj):        
        ...
        for s in range(args.num_videos):
            ...
            bproc.object.sample_poses(objects_to_sample = shapenet_obj,
            ...
            placed_objects = bproc.object.sample_poses_on_surface(objects_to_sample=shapenet_obj,
            ...
            bop_bvh_tree = bproc.object.create_bvh_tree_multi_objects(shapenet_obj)
            obj = shapenet_obj[0].blender_obj
            ...
            # poi = bproc.object.compute_poi(np.random.choice(shapenet_obj, size=1, replace=False))
            poi = np.array([0, 0, 0])
            cam_locs = location_circle(camloc_init, [0, 0, 0], args.num_imgs)
            cam_poses = 0
            ...
            obj_name = shapenet_obj[0].blender_obj.name
            for dic in seg_data['instance_attribute_maps'][0]:
                if dic['name'] == shapenet_obj[0].blender_obj.name:
                    obj_id = dic['idx']
            seg_map = []
            ...
            bproc.writer.write_air(os.path.join(args.output_dir, args.synset_id),
                                target_objects = shapenet_obj,
                                dataset = '{}_{:02d}'.format(source_id, s),
                                depth_scale = 0.1,
                                depths = data["depth"],
                                colors = data["colors"],
                                semantic = seg_map,
